...
import { LoginData, RegisterData, ChangeData, AuthContextData } from '../types';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
    ...
    const register = async ({ name, email, password, cpf, setErrorMsg }: RegisterData) => {
        try {
            const res = await axiosAPI.post('users/register/', { name, email, password, cpf });
            if (res.status === 201) {
    ...
    const change = async ({ oldPassword, newPassword, setErrorMsg }: ChangeData) => {
        try {
    ...
    return (
        <AuthContext.Provider value={{ login, logout, change, register }}>
            {children}
